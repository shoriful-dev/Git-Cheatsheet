...
export default function Sidebar({ items, isOpen, setIsOpen }) {
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={`#${itemId}`}
                      onClick={() => setIsOpen(false)}
                      className={`group flex w-full items-center rounded-lg px-3 py-[7px] text-[13px] font-medium transition-all duration-150 relative ${
                        isActive
      ...
      <div className="p-3 border-t border-slate-800/50">
        <div className="px-3 py-2.5 rounded-lg bg-slate-900/50 border border-slate-800/50">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-[10px] font-bold text-blue-400">v1.0</span>
          </div>
          <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 w-full rounded-full"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      {/* Desktop */}
      <aside className="fixed left-0 top-[57px] z-30 hidden h-[calc(100vh-57px)] w-[260px] border-r border-slate-800/50 bg-slate-950/95 backdrop-blur-sm lg:block">
        {sidebarContent}
      </aside>
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-50 h-full w-[280px] bg-slate-950 border-r border-slate-800 shadow-2xl shadow-black/50 lg:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
